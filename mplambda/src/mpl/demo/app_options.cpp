...
/*
EP_STAT mpl::demo::AppOptions::writeToGDP() const {
    json options;
    ...
    gdp_agent agent;
    agent.create("mplambda_app_options2");
    agent.open("mplambda_app_options2");
    //return agent.write("mplambda_app_options2", options);
    return 0;
}

void mpl::demo::AppOptions::loadFromGDP() {
    // TODO: What if the data is not properly formated?
    json options;
    gdp_agent agent;
    agent.open("mplambda_app_options");
    //agent.read("mplambda_app_options", -1, &options);

    scenario_= options["scenario"];
    algorithm_ = options["algorithm"];
    coordinator_ = options["coordinator"];
    jobs_ = options["jobs"];
    problemId_ = options["problemId"];
    env_ = options["env"];
    robot_ = options["robot"];
    envFrame_ = options["envFrame"];
    start_ = options["start"];
    goal_ = options["goal"];
    goalRadius_ = options["goalRadius"];
    min_ = options["min"];
    max_ = options["max"];
    timeLimit_ = options["timeLimit"];
    checkResolution_ = options["checkResolution"];
    singlePrecision_ = options["singlePrecision"];
}
*/
